...
// defining fuctionality of all the buttons present on the page
homeBtn2.onclick = () => {
...
// full game function (contains all the necessary functions to run this game)
function fullGameStart() {
  // setting up things to start the game properly each time
  clicks = 0;
  text.textContent = "Click or Press Spacebar to Start";
  colorChangeArea.style.background = "rgba( 255, 255, 255, 0.4 )";
  reactTime.style.visibility = "hidden";

  //actual game start function (color changing functionality)
  function startgame() {
    if (clicks == 1) {
  ...
  // functionality of the game if the player clicks before the color changes
  function tooSoon() {
  ...
  // fuctionality when the player actually plays (clicks)
  colorChangeArea.onclick = () => {
    clicks++;
    // console.log(clicks)
    startgame();
    tooSoon();
    //fucntionality if the player clicked at right time (reaction time is measured and game reset functionality also)
